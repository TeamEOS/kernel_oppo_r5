...
int bq24196_set_input_chg_current(struct opchg_charger *chip, int iusbin_mA, bool aicl)
{
	u8 i=0,j=0,k=0;
	int chg_vol;
	int aicl_count;
	...
	//debug for mtbf,bq24196 do not use aicl
	aicl = false;
	#endif
...
int bq24196_hw_init(struct opchg_charger *chip)
{
    ...
    rc = bq24196_set_prechg_current(chip, 450);
...
void bq24196_dump_regs(struct opchg_charger *chip)
{
	...
	for (addr = 0; addr <= BQ24196_LAST_CNFG_REG; addr++) {
		...
		if (rc) {
			...
			pr_debug("bq24196_read_reg 0x%02x = 0x%02x\n", addr, reg);
		}
	}
...
int bq24196_chg_uv(struct opchg_charger *chip, u8 status)
{
    ...
    if (status != 0) {
		...
		//opchg_config_input_chg_current(chip, INPUT_CURRENT_BY_POWER, 100);
