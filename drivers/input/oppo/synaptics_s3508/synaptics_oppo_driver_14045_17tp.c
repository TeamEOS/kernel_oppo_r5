...
#define TPD_DEVICE "synaptics-rmi14045"
...
static void enable_aggressive_segmentation(bool enable)
{
	...
	if(!ts_g)
		return;
	if(ts_g->suspended || ts_g->i2c_suspend)
		return;
	mutex_lock(&ts_g->mutex);
	ret = synaptics_rmi4_i2c_write_byte( ts_g->client, 0xff, 0x04); 
	if( ret < 0 ){
		TPD_ERR("synaptics_read_register_map: failed for page select\n");
		goto ERR;
	}
	if(enable){
		ret = synaptics_rmi4_i2c_write_byte( ts_g->client, 0x0A, 0x00); 
	}else{
		ret = synaptics_rmi4_i2c_write_byte( ts_g->client, 0x0A, 0x02); 
	}
...
static  void get_tp_id(int TP_ID1,int TP_ID2,int TP_ID3, struct synaptics_ts_data *ts)
{
	...
	if(TP_ID1  >= 0) {
		...
		//ts->id_pullup = pinctrl_lookup_state(ts->pinctrl, "id_pullup")
		//if(!ret)		
...
static int tp_esd_check(void)
{
	...
	if(ret < 0)
		goto ERR;
	ret = synaptics_rmi4_i2c_read_word(ts_g->client, 0xE2);
...
extern int (*tp_esd_check_fn)(void);
