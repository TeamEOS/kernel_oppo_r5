...
int ts4621_reg_write(u8 reg, u8 value)
{
    ...
    retval = ts4621_reg_read(reg, &old_value);
	pr_debug("after write value = 0x%08X  reg no. = 0x%02X\n", old_value, reg);
			   ...
			   const struct i2c_device_id *id)
{
    ...
    pr_debug("exit %s err = %d", __func__, err);
	return err; 
}
...
void  ts4621_amp_on(int on, u8 gain)
{
	if (on) {
        ...
        pr_debug("ts4621_amp_on Success 1");
        ...
        pr_debug("ts4621_amp_on Success 2");
    }
    return;
}
