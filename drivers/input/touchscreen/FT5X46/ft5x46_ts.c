...
//		regulator_put(data->pdata->vcc_i2c);		
...
static ssize_t focaltech_update_fw_store(struct device *dev,struct device_attribute *attr,const char *buf, size_t count)
{
	...
	sprintf(data->fw_version,"150050%d",uc_host_fm_ver);
	
	pr_err("focaltech_update_fw,old fw_ver = %d,new fw_ver = %d,fileval = %ld,tp_fw_ok = %d\n",data->fw_ver[0],uc_host_fm_ver,fileval,tp_fw_ok);
	if((fileval == 1)||(tp_fw_ok !=1)||(uc_host_fm_ver != data->fw_ver[0]))	{		
		...
		if (!data->loading_fw) {
			...
			if(fw_upgrade_ok){
				i_ret = request_threaded_irq(data->client->irq, NULL,
					focaltech_ts_interrupt,
					data->pdata->irqflags | IRQF_ONESHOT | IRQF_TRIGGER_FALLING,
...
static int focaltech_ts_probe(struct i2c_client *client,const struct i2c_device_id *id)
{
	...
	focaltech_i2c_Read(client, &reg_addr, 1, &reg_value, 1);
	if (err < 0){
	...
	sprintf(data->fw_version, "150050%d",data->fw_ver[0] );		
